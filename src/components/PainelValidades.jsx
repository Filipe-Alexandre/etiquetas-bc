...
export function PainelValidades({ todosProdutos, fecharPainel, recarregarDados }) {
  ...
  const [mostrarApenasAlertas, setMostrarApenasAlertas] = useState(false); // NOVO ESTADO DE FILTRO

  // NOVA MÁSCARA DD/MM/AAAA (8 dígitos numéricos)
  ...
  const salvarTudo = () => {
    ...
    if (temMudanca) {
      ...
      recarregarDados();
      setValidadesEditadas({}); // Limpa os campos editados após salvar
    } else {
      alert("Nenhuma validade foi alterada para salvar.");
    }
  };

  // FUNÇÃO PARA CALCULAR O STATUS (Suporta DD/MM/AA e DD/MM/AAAA)
  const obterStatusValidade = (dataStr) => {
    if (!dataStr || dataStr.length < 8) return 'normal';
  ...
  // LÓGICA DE FILTRAGEM (BUSCA + CHECKBOX DE ALERTAS)
        ...
        <div style={{ display: 'block',marginBottom: '20px'}}>
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <input 
                ...
                style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '4px', width: 'auto', fontFamily: 'var(--bold)', flexGrow: 1, minWidth: '200px', outline: 'none' }}
            />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {produtosFiltrados.length === 0 ? (
                  <tr>
                      <td colSpan="5" style={{ textAlign: 'center', padding: '40px', color: '#999' }}>Nenhum produto encontrado com estes filtros.</td>
                  </tr>
              ) : (
                  produtosFiltrados.map(prod => {
                    ...
                    if (status === 'vencido') {
                        ...
                        textColor = '#c62828';
                        tagStatus = <span style={badgeVencidoStyle}>VENCIDO</span>;
                    } else if (status === 'alerta') {
                        rowStyle = { backgroundColor: '#fff8e1', borderBottom: '1px solid #ffecb3' };
                        textColor = '#f57f17';
                        tagStatus = <span style={badgeAlertaStyle}>VENCE EM BREVE</span>;
                    }
                    ...
                    return (
                      <tr key={prod.id} style={rowStyle}>
                        ...
                        {/* CÉLULA DO PRODUTO (NOME EM CIMA, BADGE EMBAIXO) */}
                        ...
                        <td style={tdStyle}>
                          <input 
                            ...
                            style={{ ...inputStyle, borderColor: status === 'normal' ? '#ccc' : textColor, color: textColor }}
                          />
                        </td>
...
// ESTILOS DAS BADGES (Sem Emojis)
const badgeVencidoStyle = { fontSize: '10px', background: '#c62828', color: 'white', padding: '3px 8px', borderRadius: '4px', fontWeight: 'bold', display: 'inline-block', letterSpacing: '0.5px' };
const badgeAlertaStyle = { fontSize: '10px', background: '#f57f17', color: 'white', padding: '3px 8px', borderRadius: '4px', fontWeight: 'bold', display: 'inline-block', letterSpacing: '0.5px' };