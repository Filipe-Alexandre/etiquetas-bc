...
export function EtiquetaKitDePor({ todosProdutos, bancoDeDados, discountType, discountValue, kitsParaImpressao, setKitsParaImpressao }) {
    ...
    const salvarKitParaImpressao = () => {
        if (!kitName) {
            alert("Dê um nome ao kit promocional antes de salvar!");
            return;
        }

        const gramaturaTotalCalc = rows.reduce((acc, row) => {
        ...
        const valorDeCalc = rows.reduce((acc, row) => {
            const prod = todosProdutos.find(p => p.id === row.id);
        ...
        if (discountType === 'percent') {
            valorPorCalc = valorDeCalc - (valorDeCalc * (discountValue / 100));
        } else if (discountType === 'value') {
            valorPorCalc = valorDeCalc - discountValue;
        }
    ...
    if (discountType === 'percent') {
        editorValorPor = editorValorDe - (editorValorDe * (discountValue / 100));
    } else if (discountType === 'value') {
        editorValorPor = editorValorDe - discountValue;
    }
    if (editorValorPor < 0) editorValorPor = 0;

    // MOTOR DE PAGINAÇÃO CORRIGIDO: Tabela = 5 slots. Máx 12 por página.
    const paginasA4 = [];
    let paginaAtual = [];
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            ...
            <div className="menu-montagem hide-print" style={{ backgroundColor: 'var(--bg-sidebar)', padding: '25px', borderRadius: '12px', border: '1px solid #ddd', width: '21cm', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                <div style={{ textAlign: 'center', color: 'var(--marrom)', fontWeight: '900', marginBottom: '20px', fontSize: '14pt' }}>
                    MENU DE MONTAGEM (KIT DE/POR)
                </div>
                ...
                <div className="print-controls" style={{ display: 'flex', gap: '20px', background: '#fff', padding: '15px 20px', border: '1px solid #ccc', marginBottom: '20px', justifyContent: 'center', borderRadius: '8px' }}>
                    <label style={{ fontWeight: 'bold', fontSize: '10pt', color: 'var(--marrom)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        Qtd. Tabelas:
                        <input type="number" min="0" value={qtdTabelas} onChange={e => setQtdTabelas(Number(e.target.value))} style={{ width: '50px', padding: '6px', textAlign: 'center', border: '1px solid #ccc', borderRadius: '4px' }} />
                    </label>
                    <label style={{ fontWeight: 'bold', fontSize: '10pt', color: 'var(--marrom)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        Qtd. Etiquetas Menores:
                        <input type="number" min="0" value={qtdEtiquetas} onChange={e => setQtdEtiquetas(Number(e.target.value))} style={{ width: '50px', padding: '6px', textAlign: 'center', border: '1px solid #ccc', borderRadius: '4px' }} />
                    </label>
                </div>

                <input
                    type="text"
                    className="search-kit-input"
                    placeholder="🔍 BUSCAR E FILTRAR PRODUTOS DO KIT..."
                    value={buscaProd}
                ...
                <div className="etiqueta kit depor-theme" style={{ border: '2px dashed var(--marrom)' }}>
                    <div className="kit-header" style={{ display: 'flex', gap: '10px' }}>
                        <input
                            type="text"
                            ...
                            style={{ borderBottom: '1px solid rgba(255,255,255,0.5)', flex: 1, minWidth: 0 }}
                        />
                                    ...
                                    <div className="col-produto" style={{ flexWrap: 'wrap' }}>
                                        ...
                                        <div className="row-controls" style={{ flex: '1 1 100%' }}>
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    const n = [...rows];
                                                    n[index].qtd = Number(e.target.value);
                                                    setRows(n);
                                                }}
                                                className="qtd-input"
                                            />
                                    ...
                                    <div className="col-validade text-center validade-box">
                                        {prod ? (
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    const n = [...rows];
                                                    n[index].validade = mascaraData(e.target.value);
                                                    setRows(n);
                                                }}
                                                style={{ borderBottom: '1px dotted var(--marrom)' }}
                                            />
                                        ) : ''}
                                    </div>
                    ...
                    <div className="kit-footer">
                        <div className="kit-warning">
                            ...
                            <p>Infos nutricionais e alergênicos,<br />consulte a embalagem</p>
                        </div>
                        ...
                        <div className="kit-barcode-area">
                            ...
                            {barcodeValue && (
                                <div className="barcode-display">
                                    <Barcode value={barcodeValue} width={1.2} height={20} fontSize={10} background="#ffffff" margin={1} displayValue={true} />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="kit-total-area depor-mode">
                            <div className="total-de">
                                ...
                                <span className="valor-de">{formataPreco(editorValorDe)}</span>
                            </div>
                            <div className="total-por">
                                <span className="texto-por">POR R$</span>
                                <span className="valor-por">{formataPreco(editorValorPor)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                    <button className="btn-action btn-orange" style={{ flex: 1, padding: '15px' }} onClick={salvarKitParaImpressao}>
                        <i class="fa-solid fa-floppy-disk"></i> ADICIONAR ESTE KIT À FOLHA
                    </button>
                    {kitsParaImpressao.length > 0 && (
                        <button className="btn-action btn-brown" onClick={() => setKitsParaImpressao([])} style={{ width: '200px' }}>
                            <i class="fa-solid fa-trash"></i> LIMPAR FOLHAS
                        </button>
                    )}
                </div>
            </div>
            ...
            {paginasA4.length === 0 && (
                <div className="preview-folha hide-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <p style={{ color: '#999' }}>Nenhum kit adicionado. Monte acima e clique em "Adicionar".</p>
                </div>
            )}

            {paginasA4.map((pagina, idxPagina) => (
                <div key={idxPagina} className="preview-folha" style={{
                    display: 'flex',
                    flexWrap: 'wrap',
                            ...
                            return (
                                <div className={`etiqueta kit ${isPromo ? 'depor-theme' : ''}`} key={`print-tab-${idxPagina}-${idxItem}`} style={{ width: '100%', maxWidth: '12cm', margin: '0 auto', pageBreakInside: 'avoid' }}>
                                            ...
                                            return (
                                                <div className="kit-row" key={idxProd} style={{ padding: '4px 0', minHeight: '24px' }}>
                                                    <div className="col-produto">
                                                        {prod && <span className="row-number">{idxProd + 1}</span>}
                                                        <div>
                                                            {prod ? `${row.qtd > 1 ? `${row.qtd}x ` : ''}${prod.complemento} ${prod.gramatura}` : ''}
                                                        </div>
                                                    </div>
                                                    <div className="col-validade text-center">
                                                        {prod ? (row.validade || (prod.validade ? mascaraData(prod.validade) : '')) : ''}
                                                    </div>
                                                    <div className="col-preco text-center">
                                                        {prod ? `R$ ${formataPreco(prod.precoBase * row.qtd)}` : ''}
                                                    </div>
                                                </div>
                                            );
                                        })}
                                    ...
                                    <div className="kit-footer">
                                        ...
                                        {item.kit.barcode && (
                                            <div className="barcode-display">
                                                <Barcode value={item.kit.barcode} width={1.2} height={20} fontSize={10} background="#ffffff" margin={1} displayValue={true} />
                                            </div>
                                        )}
                                        {isPromo ? (
                                            <div className="kit-total-area depor-mode">
                                                <div className="total-de">
                                                    <span className="texto-de">DE R$</span>
                                                    <span className="valor-de">{formataPreco(item.kit.valorDe)}</span>
                                                </div>
                                                <div className="total-por">
                                                    <span className="texto-por">POR R$</span>
                                                    <span className="valor-por">{formataPreco(item.kit.valorPor)}</span>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="kit-total-area">
                                                ...
                                                <div className="total-box">
                                                    ...
                                                    <span className="valor-total">{formataPreco(item.kit.total)}</span>
                                                </div>
                                            </div>
                                        )}
                                    </div>
