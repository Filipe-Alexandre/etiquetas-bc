...
export function EtiquetaKit({ todosProdutos, bancoDeDados, kitsParaImpressao, setKitsParaImpressao }) {
    ...
    const salvarKitParaImpressao = () => {
        if (!kitName) {
            alert("Dê um nome ao kit antes de salvar!");
            return;
        }
    ...
    // Isso garante exatas 2 tabelas por página, sem "vazar" pela borda.
    const paginasA4 = [];
    let paginaAtual = [];
                        ...
                        <div className="kit-total-area">
                            ...
                            <div className="total-box">
                                ...
                                <span className="valor-total">
                                    {formataPreco(rows.reduce((acc, r) => acc + (todosProdutos.find(p => p.id === r.id)?.precoBase || 0) * r.qtd, 0))}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            {paginasA4.map((pagina, idxPagina) => (
                <div key={idxPagina} className="preview-folha" style={{ 
                    ...
                    {pagina.map((item, idxItem) => {
                        ...
                        if (item.tipo === 'tabela') {
                            return (
                                <div className="etiqueta kit" key={`print-tab-${idxPagina}-${idxItem}`} style={{ width: '100%', maxWidth: '12cm', margin: '0 auto', pageBreakInside: 'avoid' }}>
                                    <div className="kit-header" style={{ display: 'flex', gap: '10px' }}>
